...
(function(watermark){
    ...
    var loadMark = function(settings) {
        var defaultSettings={
            ...
            watermark_alpha:0.09,//水印透明度，要求设置在大于等于0.003
        ...
        if (window.watermarkdivs && window.watermarkdivs.length > 0) {
             document.getElementsByTagName("body")[0].removeChild(document.getElementById("otdivid"));
            window.watermarkdivs = [];
        }
 
        //获取页面最大宽度
      //  var page_width = Math.max(document.body.scrollWidth,document.body.clientWidth);
        var page_width = document.documentElement.clientWidth-50;
        //获取页面最大长度
       // var page_height = Math.max(document.body.scrollHeight,document.body.clientHeight);
        var page_height = document.documentElement.clientHeight-100;
        ...
        var y;
        // 判断登录用户
        if(defaultSettings.watermark_user != undefined && defaultSettings.watermark_user != null){
          for (var i = 0; i < defaultSettings.watermark_rows; i++) {
            ...
            for (var j = 0; j < defaultSettings.watermark_cols; j++) {
              ...
              mask_div.innerHTML="<div>"+defaultSettings.watermark_user+"</div><p>"+defaultSettings.watermark_txt+"</p>";
              // if(defaultSettings.watermark_user != undefined && defaultSettings.watermark_user != null){
              //   mask_div.innerHTML="<div>"+defaultSettings.watermark_user+"</div><p>"+defaultSettings.watermark_txt+"</p>";
              // }else{
              //   mask_div.appendChild(document.createTextNode(null));
              // }

              //设置水印div倾斜显示
              // 这个属性会导致 水印消失 ie9 会自动添加这个属性 ie10 不会添加这个属性
        ...
        //一次性添加到document中
        document.getElementsByTagName("body")[0].appendChild(oTemp);
    };
